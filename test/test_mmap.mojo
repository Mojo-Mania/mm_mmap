...
def test_map_whole_file_default_length() raises:
    ...
    with open(tmp.name, "r") as f:
        ...
        assert_equal(len(m), len(content))
        assert_equal(Int(m.bytes()[9]), Int(content[9]))
    tmp.close()
...
def test_map_fd() raises:
    var s = String("raw descriptor")
    var content = s.as_bytes()
    var tmp = NamedTemporaryFile("rw")
    tmp.write_bytes(content)

    with open(tmp.name, "r") as f:
        var m = MemoryMap.map_fd(
            f.handle, len(content), prot=PROT_READ, flags=MAP_PRIVATE
        )
        assert_equal(len(m), len(content))
        assert_equal(Int(m.bytes()[0]), Int(content[0]))
    tmp.close()
