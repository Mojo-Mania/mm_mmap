...
This document explains why the API looks the way it does. It started life as a
proposal to add memory mapping to the Mojo standard library (`os.MemoryMap`);
that proposal was not accepted, so the implementation lives here as a standalone
package. The rationale below is unchanged apart from the module path.

## Summary

`mm_mmap` provides POSIX memory mapping: an owning `MemoryMap` type that maps a
file or fresh anonymous memory into the process address space, plus the
underlying `mmap`/`munmap`/`msync` bindings and typed `Prot`/`MapFlags` flag
sets. This lets Mojo programs read and write file bytes directly (no explicit
`read`/`write`), build zero-copy IPC/shared-memory buffers, and back custom
allocators.

## Motivation

Memory mapping is a fundamental OS primitive that the Mojo stdlib lacks (there
is no `mmap`, `MAP_*`, or `PROT_*` anywhere in `std/`). Real uses that are
blocked or forced into per-project FFI today:
...
Every one of these otherwise reimplements the same `external_call["mmap", ...]`
plumbing, including the per-OS constant differences that are easy to get wrong
(e.g. `MAP_ANONYMOUS` is `0x20` on Linux but `0x1000` on macOS, `MS_SYNC`
differs, etc.). A single well-tested type removes that footgun.

## API

```mojo
from mm_mmap import MemoryMap, PROT_READ, PROT_WRITE, MAP_SHARED
...
### 1. Typed flag sets (`Prot`, `MapFlags`)
...
This differs from the stdlib, which today expresses flags either as loose
module-level integers (`O_RDONLY` in `io/file.mojo`) or as integer-valued struct
namespaces (`SignalCodes`, `FcntlCommands` in `sys/_libc.mojo`) — neither of
which is type-safe. Typed flags are worth it here because `mmap`'s `prot` and
...
matching `io/file.mojo`'s `O_*`. Simpler, but loses the type safety and lets
`MemoryMap.map(f, prot=MAP_SHARED)` compile.

There is deliberately no typed `OFlags`: `MemoryMap` does not open files (it
takes an already-open `FileHandle` or descriptor), and open-flags already exist
as `O_*` in the stdlib's `io/file.mojo`.
...
`map_fd` is the primitive — it takes a raw descriptor, which is what you have
when the fd came from `shm_open`, `socketpair`, or another library — and `map`
is the `FileHandle` convenience over it.
...
- Windows (`CreateFileMappingW`/`MapViewOfFile`, 64 KB allocation-granularity
  ...
  signature.
- `madvise`/`mprotect`, `MAP_FIXED`-based remapping and huge pages are natural
  follow-ups.
- The `length=-1` "whole file" default uses `lseek`; it could move to an
